...
export class AiService {
  ...
  async generateProposal(
    ...
    if (!apiKey) {
      this.logger.log("GEMINI_API_KEY not configured. Using Smart Proposal Synthesizer.");
      return this.generateFallbackProposal(trimmed, hints);
    }
    ...
    try {
      ...
      this.logger.warn(`AI Proposal Generation call failed: ${(err as Error).message}. Falling back to Smart Proposal Synthesizer.`);
      return this.generateFallbackProposal(trimmed, hints);
    }
  }
...
Return ONLY the JSON object. No markdown, no preamble, no commentary.`;
  ...
  /**
   * Deterministic, domain-aware fallback proposal synthesizer.
   * Ensures proposal generation never fails even if API key is missing or AI service is unavailable.
   */
  public generateFallbackProposal(
    requirement: string,
    hints: {
      clientName?: string;
      projectName?: string;
    ...
    const client = hints.clientName?.trim() || "the client";

    const reqLower = requirement.toLowerCase();
    const isShopify = /shopify|store|ecommerce|e-commerce|checkout|cart|theme/i.test(reqLower);
    const isAi = /ai|rag|llm|agent|automation|gpt|bot/i.test(reqLower);
    const isMobile = /mobile|ios|android|app|flutter|react native/i.test(reqLower);
    const isSaas = /saas|dashboard|crm|erp|portal|web app|admin/i.test(reqLower);

    let domainTitle = "Custom Software Solution";
    if (isShopify) domainTitle = "Shopify Storefront & E-Commerce Platform";
    else if (isAi) domainTitle = "AI Automation & Intelligent Agent System";
    else if (isMobile) domainTitle = "Mobile Application Development";
    else if (isSaas) domainTitle = "SaaS Web Application & Portal";

    const projectName = hints.projectName?.trim() || `${client !== "the client" ? `${client} — ` : ""}${domainTitle}`;

    const description = `A comprehensive, production-grade delivery of ${projectName.toLowerCase()}, configured end-to-end for ${client}. Our team will execute the engagement across structured phases with continuous demonstration cycles, robust test coverage, and complete handover documentation.`;

    const projectUnderstanding = `${client} requires a high-performance ${domainTitle.toLowerCase()} tailored to address current operational bottlenecks and accelerate growth. Based on the brief, the objective is to eliminate manual friction, deliver a seamless user experience, and establish a reliable, scalable foundation that supports long-term business goals.`;

    const keyOutcomes = [
      "Production-ready deployment with end-to-end QA validation",
      "Streamlined user experience with reduced operational friction",
      "Complete technical handover including source code & documentation",
      "7-day post-launch warranty with dedicated technical support",
    ];

    const rawPhases = [
      {
        heading: "Phase 1: Discovery, Architecture & Strategy",
        summary: `Requirements audit, system architecture design, and technical roadmap definition for ${projectName}.`,
        deliverables: [
          "Technical Architecture & System Specification",
          "Data Model & API Interface Definition",
          "UI Wireframes & UX Flow Review",
          "Risk Assessment & Security Baseline",
        ],
        acceptance: "Technical spec and roadmap sign-off by client team. If we don't surface 3+ automation/optimisation opportunities worth more than the phase fee, the phase fee is refunded.",
        durationWeeks: 1,
        hoursEstimate: 30,
      },
      {
        heading: "Phase 2: Core Engineering & Implementation",
        summary: `Full-stack development of the key features, integrations, and business logic.`,
        deliverables: [
          "Core Application Modules & Feature Buildout",
          "API Integration & Webhooks Setup",
          "Database / Data Store Configuration & Migrations",
          "Role-based Access Control & Security Layer",
        ],
        acceptance: "Interactive staging demo of core features passed and verified.",
        durationWeeks: 3,
        hoursEstimate: 90,
      },
      {
        heading: "Phase 3: Integration, Polish & Performance Tuning",
        summary: `Refining UI/UX, third-party service connections, and high-load performance optimization.`,
        deliverables: [
          "Third-party Service & Payment Gateway Integration",
          "Mobile & Cross-Browser Responsive Testing",
          "Performance Optimization & Caching Configuration",
          "End-to-End Automated Test Suite Execution",
        ],
        acceptance: "Zero critical bugs on staging environment with target performance benchmarks met.",
        durationWeeks: 1,
        hoursEstimate: 40,
      },
      {
        heading: "Phase 4: QA Pass, Cutover & Launch Handover",
        summary: `Final security audit, production deployment cutover, and administrative handover.`,
        deliverables: [
          "Production Environment Deployment & SSL Setup",
          "Admin & User Access Handover Credentials",
          "Complete Git Repository & Setup Documentation",
          "Post-Launch Monitoring & Canary Deployment",
        ],
        acceptance: "Live deployment verified with sign-off and 7-day post-launch warranty initiated.",
        durationWeeks: 1,
        hoursEstimate: 20,
      },
    ];

    const blocks = rawPhases.map((p) => {
      const lines = [p.summary, ...p.deliverables.map((d) => `• ${d}`), `Acceptance: ${p.acceptance}`];
      return {
        ...p,
        content: lines.join("\n"),
      };
    ...
    const totalHours = blocks.reduce((sum, b) => sum + b.hoursEstimate, 0);
    const totalAmount = totalHours * hourlyRate;
    const pricing = `${symbol}${totalAmount.toLocaleString("en-IN")} · ${totalHours} hours @ ${symbol}${hourlyRate.toLocaleString("en-IN")}/hour`;

    const includedDeliverables = [
      { kind: "INCLUDED" as const, title: "Production Source Code & Repositories", description: "Full Git ownership, clean architecture, and technical documentation.", amount: Math.round(totalAmount * 0.4) },
      { kind: "INCLUDED" as const, title: "Custom Application Build & Integrations", description: "Complete feature development as specified in the Scope of Work.", amount: Math.round(totalAmount * 0.4) },
      { kind: "INCLUDED" as const, title: "QA Testing & Staging Environment", description: "Comprehensive testing on staging before live cutover.", amount: Math.round(totalAmount * 0.2) },
      { kind: "INCLUDED" as const, title: "1-Week Post-Launch Warranty", description: "Immediate fix for any bugs reported within 7 days of deployment.", amount: 0 },
    ];

    const standardExclusions = [
      { kind: "EXCLUDED" as const, title: "Hosting & Server Costs", description: "Paid directly by client to cloud providers (AWS, Vercel, Supabase, etc.).", amount: undefined },
      { kind: "EXCLUDED" as const, title: "Third-Party API & SaaS Subscriptions", description: "Billed directly by vendors (Stripe, OpenAI, Twilio, SendGrid, etc.).", amount: undefined },
      { kind: "EXCLUDED" as const, title: "Content Creation & Asset Production", description: "Copywriting, photography, and raw video assets to be provided by client.", amount: undefined },
      { kind: "EXCLUDED" as const, title: "Anything not listed above", description: "Any feature, integration, or scope addition not explicitly listed in Included Deliverables is quoted separately as a Change Request.", amount: undefined },
    ];
    ...
    return {
      ...
      projectUnderstanding,
      pricing,
      paymentTermsText: "50% Advance — Project kick-off\n30% Mid-project — End of design phase\n20% Final — On launch & handoff",
      totalHours,
      hourlyRate,
      keyOutcomes,
      blocks,
      deliverables: [...includedDeliverables, ...standardExclusions],
    };
  }
  ...
  async generateProjectPlan(input: GenerateProjectPlanInput): Promise<GeneratedProjectPlan> {
    ...
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      this.logger.log("GEMINI_API_KEY not configured. Using Smart Plan Synthesizer.");
    ...
    try {
      ...
      this.logger.warn(`AI Plan Generation call failed: ${(err as Error).message}. Falling back to Smart Plan Synthesizer.`);
      return this.generateFallbackPlan(input);
    }
  }

  public generateFallbackPlan(input: GenerateProjectPlanInput): GeneratedProjectPlan {
    const today = new Date().toISOString().slice(0, 10);
    const startDate = input.startDate && input.startDate >= today ? input.startDate : today;
      ...
      milestones: [
        {
          ...
          description: "Requirement analysis, technical specification, and setup.",
          dueDate: startDate,
        },
        {
          title: "Phase 2: Core Engineering & Implementation",
          description: "Development of primary modules and features.",
          dueDate: endDate,
        },
        {
          title: "Phase 3: Integration & Testing",
          description: "Integration testing, security audit, and deployment prep.",
          dueDate: endDate,
        },
      ],
      sprints: [
        {
          name: "Sprint 1 — Kickoff & Core",
          goal: "Build core technical foundation & architecture",
          startDate,
          endDate,
        },
      ],
      tasks: [
        { title: "Technical Requirements & Architecture Spec", milestoneIndex: 0, sprintIndex: 0, estimatedHrs: 16, priority: "HIGH" },
        { title: "Database Schema & API Interface Design", milestoneIndex: 0, sprintIndex: 0, estimatedHrs: 16, priority: "HIGH" },
        { title: "Core Backend API & Services Buildout", milestoneIndex: 1, sprintIndex: 0, estimatedHrs: 40, priority: "MEDIUM" },
        { title: "Frontend Component & Page Development", milestoneIndex: 1, sprintIndex: 0, estimatedHrs: 40, priority: "MEDIUM" },
        { title: "Third-party Integrations & Webhook Setup", milestoneIndex: 2, sprintIndex: 0, estimatedHrs: 24, priority: "MEDIUM" },
        { title: "End-to-End QA Testing & Bug Fixes", milestoneIndex: 2, sprintIndex: 0, estimatedHrs: 24, priority: "HIGH" },
      ],
    };
  }
