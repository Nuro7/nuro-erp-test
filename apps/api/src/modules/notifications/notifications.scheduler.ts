...
export class NotificationsScheduler {
  ...
  private async attendanceAbsentAndMissedPunch() {
    ...
    if (isWorkingDay && !holidayToday) {
      // Sweep every active INTERNAL user, even if HR hasn't created an
      // EmployeeProfile row yet. The previous profile-only filter meant
      // some staff were never auto-marked absent.
      const employees = await this.prisma.user.findMany({
        where: {
          status: "ACTIVE",
          roles: {
