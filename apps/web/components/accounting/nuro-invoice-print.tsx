...
const LOGO_DARK_SRC = "/logo-white.png";
const LOGO_LIGHT_SRC = "/logo-white-inverted.png";
...
function BrandLogo({
  ...
  const finalSrc = isExternal ? src : bundledSrc;
          ...
          */}
          {logo ? (
            <div className="flex items-center gap-3">
              <BrandLogo src={logo} alt={orgName} tone="dark" height={56} />
              <span className="text-3xl font-bold tracking-tight">{orgName.toUpperCase()}</span>
            </div>
          ) : (
            <BrandLogo alt={orgName} tone="dark" height={56} />
          )}
